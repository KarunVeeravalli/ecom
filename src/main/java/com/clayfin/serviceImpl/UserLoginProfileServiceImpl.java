...
import com.clayfin.repository.TempPasswordRepository;
...
public class UserLoginProfileServiceImpl implements UserLoginProfileService {
	...
	public Status register(SignupRequest signUpRequest) throws UserLoginProfileException {
    	...
    	dto.setSubject("One Time Password for Accessing your Ecom-Application");
	...
	public Status verifyOtpForRegister(OtpDto dto) throws UserLoginProfileException {
		...
		userProfile.setIsAuthorized(false);
		userProfile.setIsTempAdmin(false);
		System.out.println(user.getRoles());
		if(status.equals(Status.ACCEPTED)){
			unauathUserService.deleteUser(signUpRequest.getId());
	...
	public String login(LoginRequest loginRequest) throws UserLoginProfileException {
		...
		emailSenderService.sendLoginText(loginRequest.getEmail());
		return jwt;
	}
	...
	@Override
	public Status changePassword(PasswordDto dto) throws UserLoginProfileException {
		TempPassword tempPassword = new TempPassword();
		tempPassword.setEmail(dto.getEmail());
		tempPassword.setPasswords(encoder.encode(dto.getPassword()));
		
		UserLoginProfile user = userRepository.findByEmail(dto.getEmail());
		if(encoder.matches("password",encoder.encode(dto.getPassword()))) {
			throw new UserLoginProfileException("Password can't be the same as previous Password");
		}
		tempPasswordRepository.save(tempPassword);
		EmailDto emailDto =  new EmailDto();
		emailDto.setSubject("One Time Password for Accessing your Ecom-Application");
		emailDto.setToEmail(dto.getEmail());
		emailSenderService.sendOtp(emailDto);
		return Status.UPDATED;
	}
	@Override
	public Status verifyOtpForPasswordUpdate(OtpDto dto) throws UserLoginProfileException {
		Status status = otpService.checkOtp(dto);
		UserLoginProfile user = userRepository.findByEmail(dto.getEmail());
		TempPassword password = tempPasswordRepository.findByEmail(dto.getEmail());
		user.setPassword(password.getPasswords());
		if(status.equals(Status.ACCEPTED)){
			userRepository.save(user);
		}
		return Status.CREATED;
